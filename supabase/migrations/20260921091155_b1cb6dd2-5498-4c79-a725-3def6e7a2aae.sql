DROP FUNCTION IF EXISTS public.family_plan_status(uuid, text);

CREATE FUNCTION public.family_plan_status(_family_id uuid, _env text DEFAULT 'live')
RETURNS TABLE(is_paid boolean, next_payment_at timestamptz, cancel_at_period_end boolean, last_payment_at timestamptz)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  SELECT
    ...
    (SELECT max(current_period_start) FROM subs);
$$;
